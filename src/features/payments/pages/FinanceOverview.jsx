...
import {
  ...
  fetchPastPayments,
  settlePayments,
  fetchAdminWaitingCharges,
  fetchLatePaidWaitingCharges,
} from "../../../api/admin.api";
...
export const FinanceOverview = () => {
  ...
  const confirmSettle = async () => {
    ...
    try {
      ...
      if (financeType === "DP" && selectedGroup.orders) {
        const baseToPay = selectedGroup.orders
          .filter((o) => !o.base_settled)
          .reduce((acc, o) => acc + (o.amount || 0), 0);
        const waitingToPay = selectedGroup.orders
          .filter((o) => !o.waiting_charge_settled)
          .reduce((acc, o) => acc + (o.waiting_charge || 0), 0);
  ...
  const showDetails = (group) => {
    navigate(
      `/admin/finance/partner/${group.dp_auth_id || group.pdc_auth_id}`,
      { state: { groupData: group, financeType } },
    );
  };
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Verify earnings accounts, aggregate payouts, and settle partner
            transactions
          </p>
        </div>
      </div>
      ...
      {activeView === "pending" ? (
        ...
        <div className="space-y-6">
          ...
          <Table
            ...
            emptyMessage={
              waitingSubTab === "late"
                ? "No late paid waiting charges in the last 30 days."
                : `No ${waitingSubTab} waiting charges found in the last 4 months.`
            }
            renderRow={(item) => {
              if (waitingSubTab === "late") {
                return (
                  ...
                  >
                    ...
                    <td className="px-5 py-4 text-xs font-extrabold text-slate-900">
                      ₹ {(item.total_waiting_charge || 0).toFixed(2)}
                    </td>
                  </tr>
                );
              }
              ...
              return (
                <tr key={id} className="hover:bg-slate-50/50 transition-colors">
                  ...
                  <td className="px-5 py-4 text-xs font-extrabold text-slate-900">
                    ₹ {(item.amount_to_pay || 0).toFixed(2)}
                  </td>
                  <td className="px-5 py-4 text-xs text-slate-600">
                    Bank Info Available
                  </td>
          ...
          <Table
            ...
            renderRow={(item) => (
              ...
              >
                ...
                <td className="px-5 py-4 text-xs">
                  <Badge
                    variant={item.user_type === "DP" ? "success" : "indigo"}
                  >
          ...
          {financeType === "DP" ? (
            ...
            <div className="bg-slate-50 border border-slate-100 rounded-lg p-4 text-center">
              <span className="text-sm font-bold text-slate-700">
                Total Settlement: ₹
                {(selectedGroup.amount_to_pay || 0).toFixed(2)}
              </span>
            </div>
          )}
        </ConfirmationModal>
