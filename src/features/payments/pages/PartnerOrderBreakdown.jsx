  ...
  if (!group) {
    return (
      <div className="flex flex-col items-center justify-center h-[60vh]">
        <p className="text-slate-500 mb-4">No data found. Please navigate from the Finance Overview.</p>
  ...
  return (
    <div className="space-y-6 text-left page-transition pb-10">
      ...
      <div className="flex items-center gap-4 border-b border-slate-200 pb-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-slate-800">Order Breakdown for: {group.name}</h2>
          <p className="text-xs text-slate-400 mt-1">Review pending orders and individually settle waiting charges</p>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse min-w-[800px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="p-4 text-[10px] font-bold text-slate-500 uppercase">Locations</th>
                <th className="p-4 text-[10px] font-bold text-slate-500 uppercase text-right">Base Amount</th>
                <th className="p-4 text-[10px] font-bold text-amber-500 uppercase text-right border-l border-slate-100">Waiting Charge</th>
                <th className="p-4 text-[10px] font-bold text-slate-500 uppercase text-center">Customer Status</th>
                <th className="p-4 text-[10px] font-bold text-slate-500 uppercase text-center border-r border-slate-100">Action</th>
                <th className="p-4 text-[10px] font-bold text-brand-purple uppercase text-right">Total Owed</th>
              </tr>
            </thead>
            <tbody>
                  ...
                  <tr key={order.id} className="border-b border-slate-100 hover:bg-slate-50/50">
                    ...
                    <td className="p-4 align-top">
                      <span className="text-[11px] font-mono font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded">
                        #{String(order.order_number).slice(-8)}
                      </span>
                    </td>
                    ...
                    <td className="p-4 align-top">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-start gap-1.5">
                          ...
                          <p className="text-slate-700 font-semibold text-xs leading-tight max-w-[200px] truncate" title={order.pickup_address}>
                            {order.pickup_address}
                          </p>
                        </div>
                        <div className="flex items-start gap-1.5">
                          <div className="w-2 h-2 rounded-full bg-rose-500 mt-1 shrink-0" />
                          <p className="text-slate-500 text-[10px] leading-tight max-w-[200px] truncate" title={order.delivery_address}>
                    ...
                    <td className="p-4 text-right align-top">
                      <span className="text-sm font-bold text-slate-700 block">₹ {order.amount.toFixed(2)}</span>
                      {order.base_settled ? (
                        <span className="inline-block mt-1 text-[9px] font-bold bg-emerald-100 text-emerald-700 px-1.5 py-0.5 rounded">✓ Settled</span>
                      ) : (
                        <span className="inline-block mt-1 text-[9px] font-bold bg-amber-50 text-amber-600 px-1.5 py-0.5 rounded">Pending Bulk</span>
                      )}
                    </td>
                    ...
                    <td className="p-4 align-top text-right border-l border-slate-100 bg-slate-50/30">
                      {!hasWaiting ? (
                        ...
                        <span className="text-sm font-black text-amber-600">₹ {order.waiting_charge.toFixed(2)}</span>
                      )}
                    </td>
                      ...
                      {!hasWaiting ? (
                        ...
                        <div className="flex flex-col items-center justify-start h-full">
                          {order.waiting_charge_settled ? (
                            ...
                            <Button 
                              size="sm" 
                              variant={order.customer_paid_waiting_charge ? "primary" : "secondary"}
                    ...
                    <td className="p-4 text-right align-top">
                      <span className="text-sm font-extrabold text-brand-purple block">
                        ₹ {order.total_amount?.toFixed(2) || order.amount.toFixed(2)}
                      </span>
                    </td>
                  </tr>
                );
