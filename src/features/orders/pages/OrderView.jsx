    ...
    <div className="max-w-4xl mx-auto space-y-6 text-left page-transition">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          {order.packageDetail && (
            <div className="bg-white border border-slate-100 p-5 rounded-2xl shadow-xs">
              <h3 className="font-bold text-slate-800 text-sm mb-4">Package Details</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs text-slate-600 mb-4">
                <div>
                  <p className="text-slate-400">Description</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{order.packageDetail.product_description || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-slate-400">Product Type</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{order.packageDetail.types_of_product || 'N/A'}</p>
                </div>
                <div>
                  <p className="text-slate-400">Weight & Quantity</p>
                  <p className="font-semibold text-slate-800 mt-0.5">{order.packageDetail.product_weight} | {order.packageDetail.no_of_items} Items</p>
                </div>
                <div>
                  <p className="text-slate-400">Dimensions (L x W x H)</p>
                  <p className="font-semibold text-slate-800 mt-0.5">
                    {order.packageDetail.product_length} x {order.packageDetail.product_width} x {order.packageDetail.product_height}
                  </p>
                </div>
              </div>

              {/* Uploaded Images */}
              <h4 className="font-bold text-slate-700 text-xs mb-2">User Uploaded Images</h4>
              <div className="flex flex-wrap gap-4">
                {order.packageDetail.image1 && (
                  <img src={order.packageDetail.image1} alt="Package 1" className="w-24 h-24 object-cover rounded-lg border border-slate-200 shadow-sm" />
                )}
                {order.packageDetail.image2 && (
                  <img src={order.packageDetail.image2} alt="Package 2" className="w-24 h-24 object-cover rounded-lg border border-slate-200 shadow-sm" />
                )}
                {order.packageDetail.image3 && (
                  <img src={order.packageDetail.image3} alt="Package 3" className="w-24 h-24 object-cover rounded-lg border border-slate-200 shadow-sm" />
                )}
                {!order.packageDetail.image1 && !order.packageDetail.image2 && !order.packageDetail.image3 && (
          ...
          <div className="bg-white border border-slate-100 p-5 rounded-2xl shadow-xs overflow-hidden">
            <h3 className="font-bold text-slate-800 text-xs uppercase tracking-wide mb-3 flex justify-between items-center">
              ...
              {liveLocation ? (
                ...
                <span className="text-[10px] text-slate-400">WAITING FOR SIGNAL</span>
              )}
            </h3>
            ...
            <div className="h-48 w-full rounded-xl overflow-hidden border border-slate-100 bg-slate-50 relative z-0">
              {liveLocation ? (
                ...
                >
                  ...
                  />
                  <Marker position={[liveLocation.lat, liveLocation.lng]} icon={dpIcon}>
                    <Popup>
                      <div className="text-center">
                        <strong className="text-xs">Partner Location</strong>
                        <div className="text-[10px] text-slate-500 mt-0.5">Updated just now</div>
                      </div>
                    </Popup>
                  </Marker>
                </MapContainer>
