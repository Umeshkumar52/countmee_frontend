...
const mapOrder = (o) => {
  ...
  return {
    ...
    sender_pin_code: o.sender_pin_code || "",
    receiver_pin_code: o.receiver_pin_code || "",
    ...
    payment_status:
      o.payment_status || (o.payment_settled ? "settled" : "pending"),
...
export const fetchPendingOrders = () =>
  fetchOrdersFromUrl("/admin/pendingorders");
export const fetchAssignedOrders = () =>
  fetchOrdersFromUrl("/admin/assignedorders");
export const fetchIntransitOrders = () =>
  fetchOrdersFromUrl("/admin/intransitorders");
export const fetchDeliveredOrders = () =>
  fetchOrdersFromUrl("/admin/orders/delivered");
export const fetchBroadcastedOrders = () =>
  fetchOrdersFromUrl("/admin/orders/broadcasted");
export const fetchCancelledOrders = async () => {
  ...
  const custData =
    custRes.data.data?.orders ||
    custRes.data.data?.detail ||
    custRes.data.data ||
    [];
  const dpData =
    dpRes.data.data?.orders ||
    dpRes.data.data?.pending ||
    dpRes.data.data ||
    [];
  const merged = [
    ...(Array.isArray(custData) ? custData : []),
    ...(Array.isArray(dpData) ? dpData : []),
  ];
  return { data: merged.map(mapOrder) };
};

export const fetchPaginatedOrders = async (status, page = 1, limit = 10, orderType = null, search = "", scheduleDate = "", extraFilters = {}) => {
...
export const fetchBundleResponses = async (bundle_id) => {
  const response = await client.get(`/admin/orders/bundle-responses/${bundle_id}`);
  return response.data.data;
};
...
export const fetchBundleTracking = async (bundle_id) => {
  const response = await client.get(`/admin/orders/bundle-tracking/${bundle_id}`);
  return response.data.data;
};
